...
class ThreeLayerPlayer:
    def __init__(self, name='ThreeLayer', log_verbose=False, learning_rate=0.1, steps=100):
    ...
    def __build_model(self):
        ...
        self.layer_one_init = tf.truncated_normal(shape=(9, 3))
        self.layer_one = tf.get_variable(name='L1', initializer=self.layer_one_init)
        self.layer_one_bias = tf.get_variable(name='B1', initializer=tf.constant_initializer(value=0), shape=(3,))
        self.verbose('layer_one {}'.format(self.layer_one))

        self.layer_two_init = tf.truncated_normal(shape=(3, 6))
        self.layer_two = tf.get_variable(name='L2', initializer=self.layer_two_init)
        self.layer_two_bias = tf.get_variable(name='B2', initializer=tf.constant_initializer(value=0), shape=(6,))
        self.verbose('layer_two {}'.format(self.layer_two))

        self.layer_three_init = tf.truncated_normal(shape=(6, 9))
        self.layer_three = tf.get_variable(name='L3', initializer=self.layer_three_init)
        self.layer_three_bias = tf.get_variable(name='B3', initializer=tf.constant_initializer(value=0), shape=(9,))
        self.verbose('layer_three {}'.format(self.layer_three))
        ...
        self.layer_three_scores = tf.nn.bias_add(tf.matmul(self.layer_two_scores, self.layer_three),
                                                 bias=self.layer_three_bias)

        self.output_probabilities = tf.nn.softmax(logits=self.layer_three_scores, dim=-1)
    ...
    def train_batch(self, move_batch):
        ...
        eval_dict = {'input': self.input,
                     ...
                     'layer_three': self.layer_three,
                     'layer_three_bias': self.layer_three_bias,
                     'layer_three_scores': self.layer_three_scores,
